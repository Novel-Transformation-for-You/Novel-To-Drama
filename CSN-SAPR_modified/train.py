...
def train():
    ...
    for epoch in epoch_bar:
        ...
        epoch_bar = tqdm(total=args.num_epochs, desc='Epochs', position=0, leave=True) # 여기 변경
        for epoch in range(args.num_epochs):
            epoch_bar.update(1)  # Epoch가 시작될 때마다 업데이트
            for i, (_, CSSs, sent_char_lens, mention_poses, quote_idxes, one_hot_label, true_index, _) \
            in enumerate(tqdm(train_data, total=len(train_data), desc=f'Epoch {epoch}', unit='batch', position=0, leave=True)):
              try:
