...
def train():
    ...
    _, CSSs, sent_char_lens, mention_poses, quote_idxes, cut_css, one_hot_label, true_index, category = dev_test_iter.next()
    print('Candidate-specific segments:')
    print(CSSs)
    print('Nearest mention positions:')
    print(mention_poses)
    # print('뭐가 들어갔는지 봅시다', sent_char_lens, mention_poses, quote_idxes)
    test_test_iter = iter(test_data)
    print('##############TEST EXAMPLE#################')
    _, CSSs, sent_char_lens, mention_poses, quote_idxes, cut_css, one_hot_label, true_index, category = test_test_iter.next()
    print('Candidate-specific segments:')
    print(CSSs)
    print('Nearest mention positions:')
    print(mention_poses)
    ...
    for epoch in epoch_bar:
        ...
        for i, (_, CSSs, sent_char_lens, mention_poses, quote_idxes, cut_css, one_hot_label, true_index, _) \
            in enumerate(progress_bar(train_data, total=len(train_data), parent=epoch_bar)):
            
            try:
                # print('앞에 원문을 같이 봅시다', CSSs)
                features, tokens_list = convert_examples_to_features(CSSs, tokenizer)
                # print('features 을 보면', features)
                # print('cut_css 확인해보죠오오오오', cut_css)
                # print(len(cut_css))
        ...
        def eval(eval_data, subset_name):
            ...
            for _, CSSs, sent_char_lens, mention_poses, quote_idxes,  cut_css, _, true_index, category \
